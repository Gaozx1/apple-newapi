...
func UpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	...
	if err != nil {
		...
		if channel.ChannelInfo.IsMultiKey {
			...
			// keys is disabled, so the channel-status short-circuit must be
			// skipped for per-key operations: re-enabling key #2 of a
			// still-enabled channel has to reach the per-key status map.
			if usingKey == "" && channel.Status == status && !overridesKeyExhaustion {
				return false
			}
