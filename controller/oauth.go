...
func handleOAuthBind(c *gin.Context, providerName string, provider oauth.Provider, oauthUser *oauth.OAuthUser, flow *model.AuthFlow, state string, match model.AuthFlowMatch) (bool, bool) {
	...
	if oauthUser.AvatarURL != "" {
		if err := model.UpdateUserAvatar(identity.UserID, oauthUser.AvatarURL); err != nil {
			common.SysLog(fmt.Sprintf("[OAuth] failed to sync avatar for user %d: %s", identity.UserID, err.Error()))
		}
	}
