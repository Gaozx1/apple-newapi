...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/verification", middleware.EmailTargetRateLimit(), middleware.TurnstileCheck(), controller.SendEmailVerification)
		apiRouter.GET("/reset_password", middleware.EmailTargetRateLimit(), middleware.TurnstileCheck(), controller.SendPasswordResetEmail)
		...
		apiRouter.POST("/oauth/email/bind/start", middleware.UserAuth(), middleware.UserCriticalRateLimit("account-security"), middleware.EmailTargetFromBodyRateLimit(), middleware.DisableCache(), controller.EmailBindStart)
		apiRouter.POST("/oauth/email/bind/resend", middleware.UserAuth(), middleware.UserCriticalRateLimit("account-security"), middleware.DisableCache(), controller.EmailBindResend)
		apiRouter.POST("/oauth/email/bind", middleware.UserAuth(), middleware.UserCriticalRateLimit("account-security"), middleware.DisableCache(), controller.EmailBind)
		...
		apiRouter.GET("/oauth/telegram/login", middleware.DisableCache(), controller.TelegramLegacyAuth)
		apiRouter.POST("/oauth/telegram/bind/start", middleware.UserAuth(), middleware.DisableCache(), controller.TelegramLegacyAuth)
		apiRouter.GET("/oauth/telegram/bind/:flow_token", middleware.DisableCache(), controller.TelegramLegacyAuth)
		...
		apiRouter.POST("/verify", middleware.UserAuth(), middleware.UserCriticalRateLimit("security-verification"), middleware.DisableCache(), controller.UniversalVerify)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			userRoute.POST("/register", middleware.AccountLoginRateLimit(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.Register)
			userRoute.POST("/login", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, middleware.TurnstileCheck(), controller.Login)
			userRoute.POST("/login/2fa", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.Verify2FALogin)
			userRoute.POST("/passkey/login/begin", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.PasskeyLoginBegin)
			userRoute.POST("/passkey/login/finish", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.PasskeyLoginFinish)
			userRoute.GET("/login/encryption-key", middleware.DisableCache(), controller.GetPasswordEncryptionKey)
			userRoute.POST("/login/verify", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.VerifyLogin)
			userRoute.POST("/login/passkey/begin", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.LoginPasskeyBegin)
			userRoute.POST("/login/passkey/finish", middleware.AccountLoginRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.LoginPasskeyFinish)
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.PUT("/self", middleware.DisableCache(), controller.UpdateSelf)
				...
				selfRoute.POST("/token", middleware.DisableCache(), controller.GenerateAccessToken)
				selfRoute.DELETE("/token", middleware.DisableCache(), controller.RevokeAccessToken)
				...
				selfRoute.POST("/transfer", controller.TransferQuota)
				...
				selfRoute.POST("/lottery/draw", middleware.TurnstileCheck(), controller.DrawLottery)

				// Ticket (工单) routes — self
				...
				// OAuth 2.0 client management (user-level)
				selfRoute.GET("/oauth2/clients", controller.UserOAuthClientList)
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				ticketAdminRoute := adminRoute.Group("/admin-tickets")
				{
					...
					ticketAdminRoute.POST("/:id/close", controller.CloseTicket)
				}
		...
		oauthClientRoute := apiRouter.Group("/oauth2/client")
		oauthClientRoute.Use(middleware.AdminAuth())
		{
			oauthClientRoute.GET("/", controller.OAuthClientList)
